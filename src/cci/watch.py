...
class WatchManager:
    ...
    def start_recording(self) -> SessionContext:
        ...
        with self._state_lock:
            ...
            session_id = f"{self._session_seq:02d}_session_{timestamp.strftime('%Y%m%d_%H%M%S')}"
    ...
    def process_session(self, session: SessionContext) -> Path:
        ...
        self._logger.info(
            "Extracted %d records to %s",
            record_count, raw_path
        )
        
        # Run merge if records exist
        if record_count > 0:
            from cci.merger import merge_streams
            
            merged_path = session_dir / "merged.jsonl"
            try:
                ...
                split_dir = session_dir / "split_output"
                split_stats = split_records(merged_path, split_dir)
                self._logger.info(
